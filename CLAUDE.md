...
**Last completed:** Day 14 — `BuildingBlocks.Messaging`: `ProcessedEvent` + `IProcessedEventStore` (consuming-side counterpart to the outbox — `IsProcessedAsync` is a cheap pre-check, not the real safety net; the actual guarantee is the implementer writing the `ProcessedEvent` row in the same transaction as the handler's writes with a unique constraint on `EventId`, documented explicitly since this library can't enforce that itself) and `IdempotentEventDispatcher` (`HandleAsync` — skip if already processed, otherwise run the handler and mark it; handler exceptions are never swallowed, they propagate for the consumer's own retry/DLQ policy to decide). Also closed two real gaps in Day 13's test coverage: the `ExecuteAsync` polling loop itself was never exercised (only `DispatchBatchAsync` directly) — now tested via `StartAsync`/`StopAsync` with a 30ms poll interval, stable across repeated runs; batch size propagation to `IOutboxStore` wasn't checked — now is. 10 new tests (30 total across `BuildingBlocks.Common` + `.Messaging`), all passing. Full solution builds clean (0 warnings, 32 projects). `BuildingBlocks.Messaging` is now feature-complete per the tutorial's Phase 4 spec. Committed locally to `main`, not yet pushed.
