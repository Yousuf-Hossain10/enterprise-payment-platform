...
**Last completed:** Day 9 — added NGINX Ingress and the Prometheus/Grafana/Loki observability stack (`kube-prometheus-stack` + `loki-stack`) to `scripts/bootstrap.sh`/`.ps1`. **Significant infra detour along the way:** the host's Docker Desktop Kubernetes was defaulting to 6 nodes (1 control-plane + 5 workers) with insufficient memory allocated, causing sustained `MemoryPressure`, multi-minute API latency, and eventually an unresponsive engine — resolved by increasing Docker Desktop to 7GiB RAM / 6 CPUs and resetting Kubernetes down to a single node (documented in `docs/Deployment-Strategy.md` under "Node count and resources"). The reset wiped the cluster; Days 7-9's infra was fully reinstalled and reverified afterward. Also hit and fixed a real Grafana/Loki conflict — `loki-stack`'s chart defaults `loki.isDefault: true`, colliding with `kube-prometheus-stack`'s own default Prometheus datasource and crash-looping Grafana (`infra/loki-values.yaml` now overrides it; documented under "Grafana + Loki: Only One Default Datasource"). Verified for real against the live single-node cluster: ran the actual `bootstrap.sh` script end-to-end (checked its real exit code, not a piped one — the first check was misleading), all 6 Helm releases `deployed`, all 12 pods `Running`. Committed locally to `main`, not yet pushed.
