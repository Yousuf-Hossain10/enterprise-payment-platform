...
**Last completed:** Day 24 — `DebitAsync` with idempotency-key check and computed balance. `DebitCommandHandler` (`Wallet.Application/Debit.cs`): idempotency-key check first (retried request is a no-op, returns current balance, no reprocessing), balance computed via `IAccountRepository.GetBalanceAsync` (sum of `LedgerEntry` rows, no stored `Balance` column), insufficient-funds check, then bumps `Account.LastModifiedAtUtc` alongside the new `LedgerEntry` insert so the Day 23 `RowVersion`/`xmin` concurrency token actually engages on save. Added `ConcurrencyConflictException` to `Wallet.Application` so the handler can catch a concurrent-write conflict without Application depending on EF Core directly — `AccountRepository` (Infrastructure) translates `DbUpdateConcurrencyException` into it, mirroring Identity.Application's existing EF-free Clean Architecture boundary. 7 new unit tests (NSubstitute: success, insufficient funds, idempotent replay, missing account, concurrency conflict, non-positive amount) plus 3 new integration tests (`WalletIntegrationTests`, real Testcontainers Postgres, zero mocks) proving idempotency-key replay is enforced by the real unique index, not just the in-handler check. Full `Wallet.Tests` suite: 12/12 passing. Full solution builds clean. Committed to `main` (`feat(wallet): implement idempotent debit with balance computation`), not yet pushed. Deliberately did not wire an HTTP controller/endpoint for Debit yet — exposing it safely means deciding service-to-service auth (`[RequirePermission]`, JWT forwarding), which `docs/Security-Model.md` §1 explicitly flags as still open; out of scope for what Day 24 literally asks for ("`DebitAsync`," not "debit endpoint").

**ADR note:** ADR authorship was reversed this session per explicit direction — Claude now drafts full ADRs (Context/Decision/Options/Trade-offs/Consequences), user reviews/accepts. Backfilled ADR-0001 (RabbitMQ vs Kafka) and ADR-0002 (saga orchestration) as standalone files from the existing worked examples, wrote ADR-0003 (monorepo vs polyrepo) in full, and completed ADR-0004 (Argon2id) and ADR-0005 (JWT + opaque refresh hybrid) — all in `docs/adr/`, all Accepted. `docs/Technology-Decisions.md`'s ADR index updated to match. A Phase 5 Learning Journal entry is still outstanding — worth writing before too much Phase 6 context displaces it.
