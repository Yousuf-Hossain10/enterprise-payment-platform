...
**Last completed:** Day 27 — the parallel-debit concurrency test. `ConcurrentDebitTests.Twenty_concurrent_debits_never_overdraw_the_account` fires 20 genuinely concurrent debits against one account, each on its own `DbContext`/`AccountRepository`/handler instance (`DbContext` isn't thread-safe) and its own idempotency key — 20 independent racing attempts, not 20 retries of one request. Asserts the non-negotiable invariant regardless of how the race resolves: final ledger balance is never negative, every reported success maps to exactly one ledger entry (no double-processing, no silently-lost writes), and the math reconciles exactly (`openingBalance - successCount * debitAmount == finalBalance`). Deliberately does *not* assert an exact success count (e.g. `floor(openingBalance / debitAmount)`) — `DebitCommandHandler` doesn't retry on `ConcurrencyConflictException` yet, so some of the 20 requests are expected to lose the race and fail with a transient "Concurrent modification - retry" even though funds were sufficient at the moment they were fired. Whether to add an in-handler retry loop so the success count converges to the theoretical maximum is explicitly Day 28's job ("harden edge cases the concurrency test surfaces"), not solved here. Ran the test 11 times back to back against real Testcontainers Postgres with zero failures before committing, confirming it isn't flaky. Full `Wallet.Tests` suite: 27/27 passing. Full solution builds clean. Committed to `main` (`test(wallet): add parallel debit concurrency test`), not yet pushed.

**Session note:** this session picked up after another interruption — Docker Desktop and the K8s cluster were both down at session start and needed ~10-15 minutes to come back up before Postgres/RabbitMQ port-forwards worked; no code was lost, Day 25 was already complete and just needed re-verification before Days 26-27 proceeded.
