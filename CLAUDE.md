...
**Last completed:** Day 34 — `Idempotency-Key` header enforcement on `POST /payments`. `PaymentsController` is Payment's first real HTTP surface: `POST /api/v1/payments` creates a payment (`Created` status) and requires an `Idempotency-Key` header — a missing header is itself a 400 (`ValidationProblem`), not an implied "generate one for me," per `API-Guidelines.md`. Replaying the same key returns the original payment (same id/body/201 status) rather than creating a second one — `CreatePaymentCommandHandler` checks first via the real unique index on `IdempotencyKey` (Day 33), which is what actually makes the guarantee safe under concurrent retries. `GET /api/v1/payments/{id}` is a plain read, no key needed. Every route requires a valid JWT (`BuildingBlocks.Security.AddPlatformJwtAuthentication()`, already built in Phase 4) — the established pattern for user-facing endpoints, distinct from the still-open internal service-to-service question (Wallet's Day 24 deferral). No `[RequirePermission]` restriction yet since no permission scheme exists for payments and Roles currently doubles as permission strings platform-wide. `IPaymentRepository` gained `GetByIdempotencyKeyAsync`/`Add`; `GetPaymentByIdQuery`/`Handler` added for a thin, repository-free controller. **Verified for real, end-to-end:** ran `Identity.Api` and `Payment.Api` together against the live cluster Postgres, registered a real user, logged in for a real JWT, then via curl confirmed 400 with no header, 201 with one, an identical response on replay, 200 on `GET`, and 401 with no bearer token. 9 new tests (7 unit + 2 real-Postgres integration). Full `Payment.Tests` suite: 57/57 passing. Full solution builds clean. Committed to `main` (`feat(payment): enforce idempotency key on payment creation`), not yet pushed.
