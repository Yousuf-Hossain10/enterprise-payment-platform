...
- **`src/BuildingBlocks/`** — the four Phase 4 shared libraries (`Common`, `Messaging`, `Observability`, `Security`), each with a `*.Tests` project alongside it. Every service references these instead of reimplementing cross-cutting concerns; nothing service-specific belongs here. Also contains `Ping.Api` (Day 16) — a throwaway service proving all four compose and boot together, not part of the deployable topology; see its own README.
